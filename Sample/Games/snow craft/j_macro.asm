...
keyplay macro
   ...
   save_register <ax>
l1:
   mov ah,1		;interrupt for keyboard
   int 16h		;call interrupt
   jz l2		;ah=scan code,al=asscii code,zf=0 buffer empty	
   mov ah,0		;read key from keyboard
   int 16h
   jmp l1
l2:
   in al,60h
...
showmessage macro x,y,string
   save_register <ax,dx>
   mov ah,02h		;set position cursor
   mov d1,x		;dl=col
   mov dh,y		;dh=row
   int 10h
   lea dx,string
